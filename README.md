...
- Use `!gun ct` or `!gun t` to configure that team's loadout even while playing on the other team (also available as `/gun ct`, `/gun t`, and `css_gun ct/t` in the console). The title shows the team being configured; primary, secondary, and pistol preferences are saved for that team. Sniper and Zeus preferences remain shared between teams. These shortcuts are available when the loadout menu is enabled, independently of the quick weapon commands.
- Press **E** on a weapon category to open its selection page. Use **Z/S** (AZERTY) or **W/S** (QWERTY) to browse all weapons allowed for that category and team; the list scrolls automatically. The focused option is yellow and the saved choice is marked `(selected)`. Press **E** to select a weapon and return to the main menu; the current weapon can also be selected. Selection pages have no Back option. The original controls remain unchanged: **R** (Reload, the default exit control) closes the entire menu from any page without changing the choice. Enemy-weapon and Zeus options still cycle on the main page.
- Only the RetakesAllocator loadout menu and its selection pages hide the A/D icons, displaying Z/S navigation buttons (W/S on QWERTY) even for long lists. E selection and R exit remain available. Other plugins retain the default A/D page icons. Install the bundled RetakesAllocator, Absynthium_MenuCore, and shared Absynthium_MenuApi together to receive this per-menu display setting.
- After selecting a weapon, the main menu focuses the category you just edited (Primary, Secondary, Pistol, or Sniper), regardless of the weapon's position in the selection list.
